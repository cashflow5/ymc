...
public class CommonUtil {
	...
	public static boolean isMobile(String mobile){
		String regexStr = "^(0|86|17951)?(13[0-9]|15[012356789]|17[05678]|18[0-9]|14[57])[0-9]{8}$";
		Pattern p = null;
        Matcher m = null;
        boolean b = false;
	...
	/**
	 * hideEmailMiddleLetter:隐藏邮箱中间位数 
	 * @author li.n1 
	 * @param email
	 * @return 
	 * @since JDK 1.6
	 */
	public static String hideEmailMiddleLetter(String email){
		StringBuffer sb = new StringBuffer(6);
		if(email.indexOf("@")-2>=0){
			...
			if(email.indexOf("@")==2){
				...
				sb.append(email.substring(sb.length(),email.indexOf("@")));
			}
		}else{
			sb.append("*");
		}
		return sb.toString()+email.substring(email.indexOf("@"));
	}
	
	/**
	 * getPasswordPower:获取密码强度，1低 2中 3高 
	 * @author li.n1 
	 * @param password
	 * @return 
	 * @since JDK 1.6
	 */
	public static String getPasswordPower(String password){
		//是否包含数字标志
		...
		for(char c : password.toCharArray()){
			...
			if(!letterFlag && Character.isLetter(c)){
				letterFlag = true;
			}
			if(!otherFlag && !(Character.isDigit(c)) && !(Character.isLetter(c))){
				otherFlag = true;
			}
		}
